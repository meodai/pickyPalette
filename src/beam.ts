...
export function createBeamManager(els: BeamElements, cbs: BeamCallbacks): BeamManager {
  const { $beamMode, $beamToken, $beamConnect, $beamCopy, $beamStatus, $ioLed } = els;
  ...
  function sendPalette(): void {
    ...
    if (!(beamSession instanceof SourceSession) || !beamSession.hasPeers()) return;
    const dp = cbs.getDisplayPalette();
    if (dp.length === 0) return;
    const tokens: Record<string, string> = {};
    dp.forEach((hex, i) => { tokens[`color-${i}`] = hex; });
  ...
  function connectTarget(): void {
    ...
    if (!token) { showError('Enter a session token'); return; }
  ...
  $beamConnect.addEventListener('click', () => {
    if ($beamMode.value === 'receive' && beamSession && beamSession instanceof TargetSession && beamSession.getState() === 'paired') {
      beamSession.disconnect();
      beamSession = null;
      initTarget();
      return;
